ALTER TABLE `co_movimenti`
CHANGE `iddocumento` `iddocumento` int(11) NULL AFTER `data`,
ADD `idvendita_banco` int(11) NULL AFTER `iddocumento`,
...
UPDATE `zz_views` SET `query` = 'IF(co_documenti.numero_esterno IS NULL, vb_venditabanco.numero_esterno, co_documenti.numero_esterno)' WHERE `id_module` = '16' AND `id` = '231';
