...
class Articoli extends Resource implements RetrieveInterface, CreateInterface
{
    ...
    {
        $query = 'SELECT id, idarticolo AS id_articolo, idintervento AS id_intervento, qta, created_at as data FROM at_righe_attivita WHERE `idarticolo` IS NOT NULL AND `idintervento` = :id_intervento';
    ...
    public function delete($request)
    {
        ...
        $database->query('DELETE FROM `at_righe_attivita` WHERE `idarticolo` IS NOT NULL AND `idintervento` = :id_intervento', [
            ':id_intervento' => $request['id_intervento'],
        ]);
    }
}
