...
?>
        <div class="col-md-3">
            ...
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title"><?php echo tr('Gerarchia attività'); ?></h3>
                </div>
                ...
                <div class="panel-body">
                    <div class="panel-group">
                        <div class="panel panel-default">
                            <?php
                                $query_subactivities = 'SELECT id
                                                        FROM at_attivita
                                                        WHERE parent = ' .prepare($id_record);
                            ...
                            <script>
                                ...
                                function openCollapse($this, activity_id, i) {
                                    ...
                                    var collapseId = $this.data('collapseid');
                                    var url = $('.url-sottoattivita').val();

                                    if ($this.hasClass('in')) {
                                        $this.removeClass('in');
                                        $('#collapse' + collapseId).html("");
                                        $('#collapse' + collapseId).collapse('hide');
                                    } else {
                                        $.get(
                                            url + '?id_module=' + id_module + '&id_record=' + id_record
                                                + '&id_activity=' + activity_id + '&i=' + collapseId,
                                            function(data) {
                                                if (data != "") {
                                                    $this.addClass('in');

                                                    $('#collapse' + collapseId).html(data);
    ...
    <div class="panel panel-primary">
        ...
        <div class="panel-body">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    {[ "type": "select", "label": "<?php echo tr('Tipo attività'); ?>", "name": "idtipointervento", "required": 1, "ajax-source": "tipiintervento-new", "value": "$idtipointervento$", "readonly": "<?php echo $record['flag_completato']; ?>" ]}
                </div>
            ...
            <div class="row">
                <?php
                    $assegnati = $database->fetchArray(
                        'SELECT CONCAT(id_assegnato, "-", tipo_assegnato) as id FROM at_attivita_assegnati WHERE id_intervento = '.prepare($id_record)
                    );

                    $assegnati = array_column($assegnati, 'id');

                    echo '
                    <div class="col-md-6">
                        {[ "type": "select", "label": "'.tr('Assegnati').'", "multiple": "1", "name": "assegnati[]", "ajax-source": "tipo_assegnati", "value": "'.implode(',', $assegnati).'", "readonly" : "0" ]}
                    </div>';
                ?>
