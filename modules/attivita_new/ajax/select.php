...
switch ($resource) {
    case 'tipiintervento-new':
        $query = 'SELECT idtipointervento AS id,
            CASE
                WHEN ISNULL(tempo_standard) OR tempo_standard <= 0 THEN descrizione
                WHEN tempo_standard > 0 THEN  CONCAT(descrizione, \' (\', REPLACE(FORMAT(tempo_standard, 2), \'.\', \',\'), \' ore)\')
                END AS descrizione, tempo_standard
            FROM at_tipiattivita |where| ORDER BY idtipointervento';
