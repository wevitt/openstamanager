...
switch (post('op')) {
    case 'update':
        ...
        if ($dbo->fetchNum('SELECT * FROM `at_fasceorarie` WHERE `nome`='.prepare($nome).' AND `id`!='.prepare($id_record)) == 0) {

            if (!empty($is_predefined)) {
                $dbo->query('UPDATE at_fasceorarie SET is_predefined = 0');
            }

            $dbo->update('at_fasceorarie', [
                'nome' => $nome,
    ...
    case 'add':
        ...
        if ($dbo->fetchNum('SELECT * FROM `at_fasceorarie` WHERE `nome`='.prepare($nome)) == 0) {

            $dbo->insert('at_fasceorarie', [
                'nome' => $nome,
                'ora_inizio' => $ora_inizio,
                'ora_fine' => $ora_fine,
            ]);
            ...
            $tipi_intervento = $dbo->select('at_tipiattivita', '*');
    ...
    case 'delete':
        ...
        $dbo->delete('at_fasceorarie_tipiattivita', ['idfasciaoraria' => $id_record]);
