...
$registra_movimento = $dbo->selectOne('co_registra_movimenti', '*', ['id' => $id_record]);
$id_scadenze = $_GET['documento'];
$banca_impostata = Banca::find(get('banca'));

if(!empty($registra_movimento['codice_abi'])){
    $idmastrino = $dbo->fetchOne('SELECT idmastrino FROM co_movimenti_modelli WHERE id=(SELECT id_modello FROM co_movimenti_abi WHERE codice='.prepare($registra_movimento['codice_abi']).')')['idmastrino'];
} else{
    $idmastrino= '';
}
...
foreach ($id_scadenze as $id_scadenza) {
    if($id_scadenza!="null"){
        ...
        $righe_documento[] = [
            ...
            'id_conto' => $banca_impostata->id_pianodeiconti3 ?: null,
        ...
        $descrizione = $registra_movimento['descrizione'];
        $id_conto_aziendale = $banca_impostata->id_pianodeiconti3 ?: setting('Conto aziendale predefinito');
        $is_importo_avere = $registra_movimento['importo'] > 0;
    
        $righe_documento[] = [
            'iddocumento' => '',
            'id_scadenza' => '',
            'id_conto' => '',
            'dare' => $is_importo_avere ? 0 : abs($registra_movimento['importo']),
            'avere' => $is_importo_avere ? abs($registra_movimento['importo']) : 0,
        ];
        ...
        $righe_documento[] = [
            ...
            'dare' => $is_importo_avere ? abs($registra_movimento['importo']) : 0,
            'avere' => $is_importo_avere ? 0 : abs($registra_movimento['importo']),
        ];
...
foreach ($id_documenti as $id_documento) {
    ...
    $id_conto_aziendale = $banca_impostata->id_pianodeiconti3 ?: ($fattura->pagamento[$conto_field] ?: setting('Conto aziendale predefinito'));
    ...
    // Riga controparte
    if(sizeof($id_scadenze)==1){
        foreach ($scadenze as $scadenza) {
            $righe_documento[] = [
                'iddocumento' => $scadenza['iddocumento'],
                'id_scadenza' => $scadenza['id'],
                'id_conto' => $id_conto_controparte,
                'dare' => $is_importo_avere ? 0 : $registra_movimento['importo'],
                'avere' => $is_importo_avere ? $registra_movimento['importo'] : 0,
            ];
            ...
            $righe_documento[] = [
                ...
                'id_conto' => $id_conto_aziendale,
                'dare' => $is_importo_avere ? $registra_movimento['importo'] : 0,
                'avere' => $is_importo_avere ? 0 : $registra_movimento['importo'],
            ];
        }
    }else{
        foreach ($scadenze as $scadenza) {
...
} elseif ($numero_scadenze == 1 && $id_scadenze[0]!="null") {
...
if ($permetti_modelli) {
	...
	<div class="row">
		<div class="col-md-12">
			{[ "type": "select", "label": "'.tr('Modello prima nota').'", "name": "modello_primanota", "value": "'.$idmastrino.'", "values": "query=SELECT idmastrino AS id, nome AS descrizione, descrizione as causale FROM co_movimenti_modelli GROUP BY idmastrino" ]}
		</div>
	</div>';
}
...
echo '
	<div class="row">
		<div class="col-md-4">
			{[ "type": "date", "label": "'.tr('Data movimento').'", "name": "data", "required": 1, "value": "'.$registra_movimento['data'].'" ]}
		</div>

		<div class="col-md-8">
			{[ "type": "text", "label": "'.tr('Causale').'", "name": "descrizione", "id": "desc", "required": 1, "value": '.json_encode($registra_movimento['descrizione']).' ]}
		</div>
    </div>';
...
include $structure->filepath('/../primanota/movimenti.php');
...
echo '
	...
	<div class="row">
		<div class="col-md-12 text-right">
			...
			<button type="button" class="btn btn-primary" onclick="add()" disabled id="add-submit">
...
function add(){
    submitAjax("#add-form", {}, function(data) {
        $.ajax({
            ...
            success: function(data) {
                $("#modals>div").modal("toggle");
                $(".riga-'.$id_record.'").fadeOut();
            },
            error: function(data) {
            }
        });
