...
switch (post('op')) {
    ...
    case 'add':
        ...
        if (count($dbo->fetchArray('SELECT idstatointervento FROM at_stati_attivita WHERE codice='.prepare($codice))) > 0) {
            flash()->warning(tr('Attenzione: lo stato attività _COD_ risulta già esistente.', [
                '_COD_' => $codice,
            ]));
        } else {
            $query = 'INSERT INTO at_stati_attivita(codice, descrizione, colore) VALUES ('.prepare($codice).', '.prepare($descrizione).', '.prepare($colore).')';
            $dbo->query($query);
    ...
    case 'delete':
        ...
        if (count($dbo->fetchArray('SELECT id FROM at_attivita WHERE idstatointervento='.prepare($id_record))) > 0) {
            $query = 'UPDATE at_stati_attivita SET deleted_at = NOW() WHERE idstatointervento='.prepare($id_record).' AND `can_delete`=1';
        } else {
            $query = 'DELETE FROM at_stati_attivita  WHERE idstatointervento='.prepare($id_record).' AND `can_delete`=1';
        }
