...
$fasce_orarie = $dbo->fetchArray("SELECT *, CONCAT (at_fasceorarie.nome, ' (', DATE_FORMAT(ora_inizio, '%H:%i'), '-', DATE_FORMAT(ora_fine, '%H:%i'), ')') AS descrizione FROM at_fasceorarie_tipiattivita INNER JOIN at_fasceorarie ON at_fasceorarie_tipiattivita.idfasciaoraria = at_fasceorarie.id   WHERE idtipointervento=".prepare($id_record));
