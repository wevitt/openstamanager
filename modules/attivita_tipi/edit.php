...
$_SESSION['current_tipo_intervento'][""] = $id_record;
...
<form action="" method="post">
    ...
    <div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"><?php echo tr('Persone/gruppi assegnati'); ?></h3>
		</div>

		<div class="panel-body">
            <?php
                $tipo_assegnati = $database->fetchArray(
                    'SELECT CONCAT(id_assegnato, "-", tipo_assegnato) as id FROM at_tipi_assegnati WHERE id_tipoattivita = '.prepare($id_record)
                );

                $tipo_assegnati = array_column($tipo_assegnati, 'id');

                echo '
                <div class="col-md-12">
                    {[ "type": "select", "label": "'.tr('Assegnati').'", "multiple": "1", "name": "tipo_assegnati[]", "ajax-source": "tipo_assegnati", "value": "'.implode(',', $tipo_assegnati).'", "readonly" : "0" ]}
                </div>';
                //"icon-after": "add|'.$module_anagrafiche['id'].'|tipoanagrafica=Tecnico&readonly_tipo=1"
            ?>
		</div>
	</div>
...
$elementi = $dbo->fetchArray(
    'SELECT `at_attivita`.`idtipointervento`, id, codice AS numero, data_richiesta AS data, "Intervento" AS tipo_documento
    FROM `at_attivita` WHERE `at_attivita`.`idtipointervento` = '.prepare($id_record).'
    UNION
    SELECT `at_attivita_tecnici`.`idtipointervento`, idintervento AS id, codice AS numero, orario_inizio AS data, "Sessione intervento" AS tipo_documento
    FROM `at_attivita_tecnici` LEFT JOIN at_attivita ON at_attivita_tecnici.idintervento=at_attivita.id WHERE `at_attivita_tecnici`.`idtipointervento` = '.prepare($id_record).'
    UNION
    SELECT `an_anagrafiche`.`idtipointervento_default` AS `idtipointervento`, idanagrafica AS id, codice, "0000-00-00" AS data, "Anagrafica" AS tipo_documento
    FROM `an_anagrafiche` WHERE `an_anagrafiche`.`idtipointervento_default` = '.prepare($id_record).'
    UNION
    SELECT `co_preventivi`.`idtipointervento`, id, numero, data_bozza AS data, "Preventivo" AS tipo_documento
    FROM `co_preventivi` WHERE `co_preventivi`.`idtipointervento` = '.prepare($id_record).'
    UNION
    SELECT `co_promemoria`.`idtipointervento`, idcontratto AS id, numero, data_richiesta AS data, "Promemoria contratto" AS tipo_documento
    FROM `co_promemoria` LEFT JOIN co_contratti ON co_promemoria.idcontratto=co_contratti.id WHERE `co_promemoria`.`idtipointervento` = '.prepare($id_record).'
